function data = cluster_cut(data, features_to_use, n_clusters, stdmax)
  % data = cluster_cut(data, features_to_use, n_clusters, stdmax)
  %
  % Runs EM on the data, using the feature list provided, as well as the
...
% ====
  ...
  try
    [W M V]       = EM_GM_compiled( X, n.clusters );
  catch
    [W M V junk]  = EM_GM( X, n.clusters );
  end
