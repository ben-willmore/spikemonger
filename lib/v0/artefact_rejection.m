function data = artefact_rejection(directory)
...
% ======================
  ...
  % spike times, in one cell
    ...
    for ii=1:n.electrodes
      ...
      allspikes.shapes{ii}        = data(ii).spikes.shapes;
    end
  ...
  % times of all spikes across all electrodes
    %keyboard;
    %{
      % old code:
          allspikes.all         = sortrows([cell2mat(allspikes.t_absolute_dt); cell2mat(allspikes.shapes)]')';
          allspikes.all_shapes  = allspikes.all(2:28,:);
          allspikes.all         = allspikes.all(1,:);
    %}
    a = struct;
      ...
      allspikes.all_shapes = nan(27,L(allspikes.all));
      for ii=1:L(a.nspikes)
        allspikes.all_shapes(:,a.sorted(2,:)==ii) = allspikes.shapes{ii}(1:27,a.sorted(3,(a.sorted(2,:)==ii)));
      end
...
while continue_excising
  ...
  % -----------
    ...
    % current event
      ...
      shapes = allspikes.all_shapes(:,  (abs(allspikes.all-tt) <= SIMULTANEOUS_EVENT_WINDOW_SIZE) );
  
    % update plots
      update_plots(to_ask,n,shapes);
  ...
  % -------------------------------------
    ...
    % follow this decision
      switch todo
        ...
        case 'm'
          ...
          sweepnum          = to_ask.sweepnum(1);
          sweep_tt_boundary = to_ask.sweep_tt_boundary(:,1);
          shapes = allspikes.all_shapes(:, allspikes.all == tt);      
          update_plots(to_ask,n,shapes);  
          fprintf('\n\n');
          continue;
    ...
    % ------------------------------------------
      ...
      to_ask.questions_remaining = L(indices_for_next_cycle);
...
%% add excision substructure to data
% ========================================
...
function todo = ask_main_question(to_ask,sort_mode,DEFAULT_ACTION_n)
  ...
  switch sort_mode
    ...
    case 'by_worst_sweep'
      ...
      todo = demandinput('      ----> ',{'k','n','b','d','K','S','q','m','y'});

      
    case 'by_worst_nss'
      titlestr1 = ['||   # CONCURRENT SPIKES = ' n2s(to_ask.nss(1)) '   ||'];
      titlestr0a = repmat('=',1,L(titlestr1));
      titlestr0b = repmat('-',1,L(titlestr1));      
      if to_ask.questions_remaining>0
        titlestr2 = ['||   # qs remaining = ' n2s(to_ask.questions_remaining) ];
        titlestr2 = [titlestr2 repmat(' ',1,L(titlestr1)-L(titlestr2)-2) '||'];
        titlestr2 = [titlestr0b '\n' titlestr2];
      else
        titlestr2 = '';
      end
      fprintf([...
      ...
      todo = demandinput('      ----> ',{'k','n','b','d','D','S','q','m','y'});
  end
end
