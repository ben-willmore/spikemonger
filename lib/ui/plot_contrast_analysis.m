...
switch compname
  case 'macgyver'
    GRIDDIR = '/data/contrast/tokens/frozen.grids/';
  case 'chai'
    GRIDDIR = '/wocka/neil/contrast/tokens/frozen.grids/';
  otherwise
...
for aa=1:L(data)
  ...
  for ii=1:L(datat)
    ...
    contrast = datat(ii).stim_params.Contrast;
...
sve = sahani_variance_explainable(datat,tt(81:1200));

% metadata
for ii=1:L(datat)
  datat(ii).tokeep = false(1,1200);
  datat(ii).tokeep(81:1088) = true;
  
  datat(ii).tokeep_prediction = false(1,600);
  datat(ii).tokeep_prediction(1089:1200) = true;
