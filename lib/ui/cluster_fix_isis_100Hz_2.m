function [C n_harmonics_to_delete] = cluster_fix_isis_100Hz_2(C,id,dirs,cols,forced_n_harmonics)
...
max_n_harmonics = floor(maxt_ms/10);
harmonics_to_show = min(max_n_harmonics,40);
maxt_isi = max_n_harmonics * 10 + 5;
maxt_to_show = harmonics_to_show * 10 + 5;
...
% peaks
peakpos = 100:100:(100*harmonics_to_show);
for ii=1:L(peakpos);
  x = 10*ii;
...
xt = [0:10:90 100:100:max(isi1.tc)];
set(gca,'xtick',xt);
...
if ~force
  ...
  fprintf([...
    ...
    '  [99]:    all\n'...
    '  [0]:     cancel\n\n']);
  n_harmonics_to_delete = demandnumberinput('           >>>  ',[0:40 99]);
