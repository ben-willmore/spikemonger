function C = cluster_fix_isis_50Hz(C,id,dirs,cols,varargin)
...
%% current distribution from 19.4-20.6ms
% ==========================================

% height at boundaries
min_t = 19.4;
max_t = 20.6;
min_tc_idx = head(find(isis.tc > min_t));
max_tc_idx = tail(find(isis.tc < max_t));
min_count = mean(round(isis.count(min_tc_idx + (-2:0)))); % average for stability
max_count = mean(round(isis.count(max_tc_idx + (0:2)))); % average for stability

% imposed distribution
tc_idx = min_tc_idx:max_tc_idx;
i = mean([min_count,max_count]) * ones(size(tc_idx));
...
dist.tt = isis.tt(min_tc_idx:(max_tc_idx+1));
...
for ss=1:L(data.set)
  ...
  for rr=1:L(data.set(ss).repeats)
    ...
    tokeep = true(1,L(spt));
    isi = diff(spt);
    problem_isi_idx = find((isi>=min_t)&(isi<=max_t));
...
[acg_long1 isi_long1] = calculate_acg(C.data(id),105);
[acg_long2 isi_long2] = calculate_acg(data,105);
...
p.bar = bar(isi_long1.tc, isi_long1.count, 'barwidth', 1, 'facecolor',col, 'linestyle', 'none');
...
xl = xlim;
ylim([0 max(max(isi_long1.count)*1.12,1)]);
yl = ylim;

% percent
pc_less_than_1ms = round(sum(isi_long1.count(tc_idx)) / sum(isi_long1.count) * 1000)/10;
str = ['< 1ms  =  ' n2s(pc_less_than_1ms) ' %'];
text(0.2,max(max(isi_long1.count)*1.1,0.95), str,'fontweight','bold','verticalalignment','top');
...
p.bar = bar(isi_long1.tc, isi_long2.count, 'barwidth', 1, 'facecolor',col, 'linestyle', 'none');
...
% percent
pc_less_than_1ms = round(sum(isi_long2.count(tc_idx)) / sum(isi_long1.count) * 1000)/10;
str = ['< 1ms  =  ' n2s(pc_less_than_1ms) ' %'];
text(0.2,max(max(isi_long1.count)*1.1,0.95), str,'fontweight','bold','verticalalignment','top');
