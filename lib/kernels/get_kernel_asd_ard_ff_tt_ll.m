function results = get_kernel_asd_ard_ff_tt_ll(grid, Y, params)
...
% ========================
  ...
  alpha.t = ones(L(w.t),1);
  alpha.f = ones(L(w.f),1);
  alpha.l = ones(L(w.l),1);
  ...
  for ii=1:3
    ...
    A.l = ( t4_v_v(XP.l, w.f, w.t) );        
    A2.l = A.l * R.l';
    ardasdcov(ii).l = get_ard_covariance_matrix( A2.l', Y, inv(R.l)'*w.l, alpha.l);
    C.l = ardasdcov(ii).l.C;
    D.l = ardasdcov(ii).l.invC;
    w.l = R.l' * inv( A2.l'*A2.l + D.l ) * A2.l' * Y';
...
% ====================================
  ...
  stage(5).w = w;
  stage(5).logE = log(0.5*( (Y-w.l'*A.l') * (Y-w.l'*A.l')' ));
