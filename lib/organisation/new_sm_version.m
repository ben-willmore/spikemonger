function new_sm_version
  ...
  switch todo
    case 0
      return;
    otherwise
      v(1).str  = fixforsed(v(1).str);
      vnew = struct;
      vnew.n    = v(todo+1).n;
      vnew.str  = v(todo+1).str;
      vnew.sedstr  = fixforsed(v(todo+1).str);

      % update version names
        eval(['!sed -i -e ''s/' v(1).str '/' vnew.sedstr '/'' lib/*.m']);
        eval(['!sed -i -e ''s/' olddate '/' newdate '/'' lib/*.m']);
        subdirs = dir('lib/*');
        subdirs = subdirs([subdirs.isdir]);
        subdirs = subdirs(3:end);
	for ii=1:L(subdirs)
	  switch subdirs(ii).name
	    case {'old.disabled','ctuning','todo'}
	     continue;
	   otherwise	    
	    try
	    eval(['!sed -i -e ''s/' v(1).str '/' vnew.sedstr '/'' lib/' subdirs(ii).name '/*.m']);      
      eval(['!sed -i -e ''s/' olddate '/' newdate '/'' lib/' subdirs(ii).name '/*.m']);      
	    catch
	    end
	  end
	end
      ...
      % copy contents
        eval(['!cp *.m *.mat old.disabled/v' vnew.str]);
        subdirs = dir('./*');
        subdirs = subdirs([subdirs.isdir]);
        subdirs = subdirs(3:end);
        for ii=1:L(subdirs)
